	...
	: BaseWidget(parent)
{
	...
	connect(recordDataChooseFileButton, &QPushButton::clicked, this, [this]
			{ CalibrationWidget::chooseFileButton_clicked(pathOfRecordData, QFileDialog::AcceptSave); });
	connect(calibrateChooseFileButton, &QPushButton::clicked, this, [this]
			{ CalibrationWidget::chooseFileButton_clicked(pathOfCalibrateData, QFileDialog::AcceptOpen); });
	connect(calibratedParameterChooseDirectoryButton, &QPushButton::clicked, this, [this]
			{ CalibrationWidget::chooseDirectoryButton_clicked(pathOfCalibratedParameter, QFileDialog::AcceptOpen); });

	connect(&timerOfProgressBar, &QTimer::timeout, this, &CalibrationWidget::updateProgressBar);
	connect(&timerOfProgressText, &QTimer::timeout, this, &CalibrationWidget::updateProgressText);
	connect(&timerOfInitializationPeriod, &QTimer::timeout, this, &CalibrationWidget::startProgressTextTimer);
}
...
QWidget *CalibrationWidget::createInstance()
{
	return new CalibrationWidget;
}

void CalibrationWidget::init()
{
        addSubscriber(IMU_SUB, "/GETjag/imu/data", 2, &CalibrationWidget::imuCallback, this);
}

void CalibrationWidget::imuCallback(const sensor_msgs::Imu::ConstPtr &imuMsg)
{
	if (readData)
	{
...
// When the run button is pressed, a function will execute the appropriate imu launch file.
void CalibrationWidget::on_imuSelectionButton_clicked()
{		
	QFileInfo fileInfo(__FILE__);  // The path to the currently open C++ file will be obtained.
	std::cout << __FILE__ << std::endl;
	QString launchFilePath = fileInfo.dir().path();

	QDir directory(launchFilePath + "/imu_launch_file");
	QStringList files = directory.entryList(QDir::Files);
        QString filePath = "'roslaunch imu_calibrater %1 limited:=true'&";
	filePath = filePath.arg(files[0]);
        //system(filePath.toLocal8Bit().data());  // launch imu.launch file in gnome terminal.
        system("roslaunch imu_calibrater lsm6dox_imu.launch limited:=true");
}

// When you press the start button for this function, data recording will begin.
void CalibrationWidget::on_startRecordButton_clicked()
{
	// system("gnome-terminal -- bash -c 'rosbag record -O LSMBOX /GETjag/imu/data limited:=true'&");
	if (started)
	{
		started = false;
...
void CalibrationWidget::chooseDirectoryButton_clicked(QLineEdit *path, QFileDialog::AcceptMode mode)
{
...
void CalibrationWidget::chooseFileButton_clicked(QLineEdit *path, QFileDialog::AcceptMode mode)
{
	QFileDialog dialog(this);
	QString fileName;
...
void CalibrationWidget::updateProgressBar()
{
...
void CalibrationWidget::updateProgressText()
{
...
void CalibrationWidget::readDataFromFile(const QString &fileName,
...
void CalibrationWidget::on_startCalibrateButton_clicked()
{
...
void CalibrationWidget::on_saveParameterButton_clicked()
{
...
bool CalibrationWidget::supportedFileTypeCheck(QString path)
{
	QFileInfo fi(path);
...
void CalibrationWidget::setIcon(QString iconName)
{
...
void CalibrationWidget::setAttitudeImage(QString imageName, QString nextImageName)
{
	char file[] = __FILE__;
	int width = 200;
	int height = 200;
...
void CalibrationWidget::on_resetCalibrationButton_clicked()
{
...
void CalibrationWidget::returnToMainWindow()
{
	sumOfProgress = 0;
	elapsedTime = 0;
	counter = 0;
...
int CalibrationWidget::createMessageBox(QString text, QString informativeText, bool button)
{
	msgBox.setText(text);
...
void CalibrationWidget::startProgressTextTimer()
{
	timerOfInitializationPeriod.stop();
