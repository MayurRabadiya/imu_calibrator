    ...
    : nh_(nh), nh_private_(nh_private), initialized_(false)
{
    ...
    // **** register publishers
    imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(ros::names::resolve("GETjag/imu") + "/data_new", 5);

    if (publish_debug_topics_)
    {
        rpy_filtered_debug_publisher_ = nh_.advertise<geometry_msgs::Vector3Stamped>(ros::names::resolve("imu") + "/rpy/filtered", 5);

        rpy_raw_debug_publisher_ = nh_.advertise<geometry_msgs::Vector3Stamped>(ros::names::resolve("imu") + "/rpy/raw", 5);
    }
    ...
    imu_subscriber_.reset(new ImuSubscriber(
        nh_, ros::names::resolve("/GETjag/imu") + "/data_without_orientation", queue_size));
